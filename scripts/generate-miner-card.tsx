/**
 * Generate a miner marketing card PNG locally (mirrors
 * /api/internal/toolbox/miner-card without hitting the API).
 *
 * Run from glow-smart-route-nextJS/ with:
 *   npx tsx scripts/generate-miner-card.tsx \
 *     --farmName "Zenithal Grange" \
 *     --zoneName "Idaho" \
 *     --minersCount 58 \
 *     --perMinerUsd 499 \
 *     --glwPerWeek 42.9 \
 *     --image ./path/to/photo.jpg \
 *     --out ./out/zenithal-miners.png
 *
 * --minersCount and --image are optional. --image accepts a local path or
 * an https URL.
 */
...
function parseArgs(argv: string[]): CliArgs {
  ...
  const required = ["farmName", "perMinerUsd", "glwPerWeek", "out"] as const;
  ...
  return {
    ...
    minersCount: map.get("minersCount") ?? null,
    perMinerUsd: map.get("perMinerUsd")!,
...
async function main() {
  ...
  const minersCountValue = args.minersCount ? parseNumber(args.minersCount) : null;
  const perMinerUsdValue = parseNumber(args.perMinerUsd);
  const glwPerWeekValue = parseNumber(args.glwPerWeek);
  ...
  const data: MinerCardData = {
    ...
    minersCount: minersCountValue != null ? formatNumber(minersCountValue, 0) : null,
    perMinerUsd: formatUsd(perMinerUsdValue),
    glwPerWeek: formatGlw(glwPerWeekValue),
    farmImageUrl,
  };
