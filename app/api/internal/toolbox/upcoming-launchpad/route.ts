...
export async function GET(request: NextRequest) {
  ...
  try {
    const url = `${getHubUrl()}/applications/marketing-launchpad`;
    const response = await fetch(url, {
      headers: buildForwardHeaders(request),
      next: { revalidate: 60, tags: ["marketing-launchpad:list"] },
    });
    ...
    const applications = (await response.json()) as MarketingLaunchpadApplication[];
    ...
    return NextResponse.json(
      {
        error:
          error instanceof Error
            ? error.message
            : "Failed to load upcoming launchpad listings",
      },
      { status: 500 },
    );
  }
