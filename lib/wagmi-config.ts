...
function isHighRiskInjectedProfile() {
  if (typeof window === "undefined") return false;
  const providers = (window as any)?.ethereum?.providers;
  return Array.isArray(providers) && providers.length >= 3;
}
...
function pickInjectedProvider(
  ...
  const providers = Array.isArray(ethereum?.providers)
    ? ethereum.providers
    : [];
  const fromProviders = providers.find((provider: any) =>
    matchesProvider(provider, predicate)
  );
  return fromProviders;
}

function getStandardConnectors() {
  return [
    injected({
    ...
    injected({
      ...
      target: {
        ...
        provider: (window) =>
          ((window as any)?.rabby as any) ??
          pickInjectedProvider(window, (provider) => provider.isRabby === true),
      },
    }),
    injected({
      ...INJECTED_CONNECTOR_OPTIONS,
      target: {
        ...
        provider: (window) =>
          pickInjectedProvider(window, (provider) => provider.isLedgerLive === true),
      },
    }),
  ];
}

function getHighRiskFallbackConnectors() {
  return [
    walletConnect({
      projectId: process.env.NEXT_PUBLIC_WALLET_CONNECT_ID!,
      showQrModal: false,
    ...
    injected({
      ...
      target: {
        id: "injected",
        name: "Browser Wallet",
        provider: (window) => (window as any)?.ethereum,
      },
    }),
  ];
}
...
export const wagmiConfig = createConfig({
  ...
  connectors:
    typeof window === "undefined"
      ? []
      : isHighRiskInjectedProfile()
      ? getHighRiskFallbackConnectors()
