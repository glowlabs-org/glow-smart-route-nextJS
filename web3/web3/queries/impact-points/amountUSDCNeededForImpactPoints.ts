...
export const amountUSDCNeededForImpactPoints = async (
  ...
  let amountUSDCNeededBN = BigInt(Math.floor(amountUSDCNeededNumber * 10 ** 6));
  ...
  while (
    ...
    if (secondEstimate > amountImpactPointsNeededBigNumber) {
      amountUSDCNeededBN = (amountUSDCNeededBN * BigInt(999)) / BigInt(1000);
    } else {
      amountUSDCNeededBN = (amountUSDCNeededBN * BigInt(1005)) / BigInt(1000);
    }
  ...
  return {
    ...
    expectedImpactPointsNumber: Number(secondEstimate / BigInt(10 ** 8)) / 1e4,
  };
};
...
const getReserves = async () => {
  ...
  const reserveGCC =
    BigInt(gccToUSDCPair.token0.address) < BigInt(gccToUSDCPair.token1.address)
      ? reserve0
      : reserve1;
...
export const findImpactPowerFromUSDC = (
  ...
  const optimalAmountToSwap = findOptimalAmountToSwap(amountUSDC, reservesUSDC);
